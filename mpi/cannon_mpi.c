...
int main(int argc, char* argv[]) 
{
	...
	**/
	...
	if (errCode == MPI_SUCCESS) {
		...
		MPI_Send_init(&data[offset(1,width,0)], 1, COLUMN, dest, tag,
				comm_cart, &sendRequestArr[sendRequestCount]);
		sendRequestCount++;

		source = dest;
		/* set starting position at ([1]-row,[width+1]-column) and place the received column */
		MPI_Recv_init(&data[offset(1,width+1,0)], 1, COLUMN, source, tag,
				comm_cart, &recvRequestArr[recvRequestCount]);
		recvRequestCount++;
	}
	...
	if (errCode == MPI_SUCCESS) {
		...
		MPI_Send_init(&data[offset(height,1,0)], 1, ROW, dest, tag,
				comm_cart, &sendRequestArr[sendRequestCount]);
		sendRequestCount++;
		source = dest;
		/* set starting position at ([height+1]-row,[1]-column) and place the received row */
		MPI_Recv_init(&data[offset(height+1,1,0)], 1, ROW, source, tag,
				comm_cart, &recvRequestArr[recvRequestCount]);
		recvRequestCount++;
	}
	...
	if (errCode == MPI_SUCCESS) {
		...
		MPI_Send_init(&data[offset(1,1,0)], 1, ROW, dest, tag,
				comm_cart, &sendRequestArr[sendRequestCount]);
		sendRequestCount++;
		source = dest;
		/* set starting position at ([0]-row,[1]-column) and place the received row */
		MPI_Recv_init(&data[offset(0,1,0)], 1, ROW, source, tag,
				comm_cart, &recvRequestArr[recvRequestCount]);
		recvRequestCount++;
	}
	...
	if (errCode == MPI_SUCCESS && 1) {
		...
		MPI_Send_init(&data[offset(height,width,0)], 1, POINT, dest, tag,
				comm_cart, &sendRequestArr[sendRequestCount]);
		sendRequestCount++;
		source = dest;
		/* set starting position at ([height+1]-row,[width+1]-column) and place the received point */
		MPI_Recv_init(&data[offset(height+1,width+1,0)], 1, POINT, source, tag,
				comm_cart, &recvRequestArr[recvRequestCount]);
		recvRequestCount++;
	}
	...
	if (errCode == MPI_SUCCESS) {
		...
		MPI_Send_init(&data[offset(1,width,0)], 1, POINT, dest, tag,
				comm_cart, &sendRequestArr[sendRequestCount]);
		sendRequestCount++;
		source = dest;
		/* set starting position at ([0]-row,[width+1]-column) and place the received point */
		MPI_Recv_init(&data[offset(0,width+1,0)], 1, POINT, source, tag,
				comm_cart, &recvRequestArr[recvRequestCount]);
		recvRequestCount++;
	}
	...
	if (errCode == MPI_SUCCESS) {
		...
		MPI_Send_init(&data[offset(height,1,0)], 1, POINT, dest, tag,
				comm_cart, &sendRequestArr[sendRequestCount]);
		sendRequestCount++;
		source = dest;
		/* set starting position at ([height+1]-row,[0]-column) and place the received point */
		MPI_Recv_init(&data[offset(height+1,0,0)], 1, POINT, source, tag,
				comm_cart, &recvRequestArr[recvRequestCount]);
		recvRequestCount++;
	}
	...
	if (errCode == MPI_SUCCESS) {
		...
		MPI_Send_init(&data[offset(1,1,0)], 1, POINT, dest, tag,
				comm_cart, &sendRequestArr[sendRequestCount]);
		sendRequestCount++;
		source = dest;
		/* set starting position at ([0]-row,[0]-column) and place the received point */
		MPI_Recv_init(&data[offset(0,0,0)], 1, POINT, source, tag,
				comm_cart, &recvRequestArr[recvRequestCount]);
		recvRequestCount++;
	}
	...
	if (myRank == 0) {
		...
		printf("persistant communtication\n");
	}
...
void swapImage(unsigned char* data,unsigned char* results)
{
	...
	for(k=0; k<depth; k++)
	{
		for(i=0;i<height+2;i++)
		{
			for(j=0;j<width+2;j++)
			{
				data[offset(i,j,k)] = results[offset(i,j,k)];
			}
		}
	}
